...
std::string ReadUserMethodOfInput() {
  std::ifstream userMethodOfInputFile(USER_METHOD_OF_INPUT_FILE);
  std::string methodOfInput;
  if (userMethodOfInputFile.is_open()) {
    std::getline(userMethodOfInputFile, methodOfInput);
    userMethodOfInputFile.close();
  }
  return methodOfInput;
}

std::string ReadUserInput() {
  std::ifstream userInputFile(USER_DIALOGUE_FILE);
  std::string userInput;
  if (userInputFile.is_open()) {
    std::getline(userInputFile, userInput);
    userInputFile.close();
  }
  return userInput;
}
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
  ...
  // Clear control.txt
  FILE *controlFile = fopen("control.txt", "w");
  fclose(controlFile);
  ...
  while (1) {
          ...
          if (controlFile == NULL) {
              ...
              controlFile = fopen("control.txt", "w");
              fclose(controlFile);
              printf("Created control.txt\n");
          } else {
              // File exists, check its content
              char buffer[10];
              if (fgets(buffer, sizeof(buffer), controlFile) != NULL) {
                  // Check if the content is "exit"
