...
local function main()
	...
	--Character.PlayVoiceOver(Character, 'greeting', "Scripts:TriggerLookAtReaction")

	if Character and Character:IsNPC() then

		npc_detected = true
		-- if npc_detected then
		-- 	return 0
		-- end
			...
			if audio_flag then
				while true do
					...
					if speakFile then
						...
						-- Check elapsed time and break if it's more than 30 seconds
						local elapsed_time = os.time() - start_time
						if elapsed_time > 30 then
							print("Breaking due to 30 seconds timeout.")
							break
						end
					
						if speakcheck then
							print("speak detected in speak.txt")
							local speakFile = io.open(speakFilePath, "w")
							if speakFile then
								speakFile:write("")
								speakFile:close()
							end
